...
function getElastiCacheEndpoints(callback) {
    elasticache.describeCacheClusters({CacheClusterId: CACHE_CLUSTER_ID, ShowCacheNodeInfo: true }, function(err, data) {
        console.log('describeCacheClusters', err, 'data', data);
        if (!err) {
            ...
            if (data.CacheClusters[0].CacheClusterStatus == 'available') {
                ...
                if (!sameEndpoints(endpoints, CACHE_ENDPOINTS)) {
                    ...
                    memcached.client = new Memcached(CACHE_ENDPOINTS);
                    setupListeners();
...
function setupListeners() {
    memcached.client.on('failure', function( details ){ sys.error( "Server " + details.server + "went down due to: " + details.messages.join( '' ) ) });
    memcached.client.on('reconnecting', function( details ){ sys.debug( "Total downtime caused by server " + details.server + " :" + details.totalDownTime + "ms")});
}
